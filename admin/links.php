...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['reset_id'])) {
        $reset = $pdo->prepare('DELETE FROM clicks WHERE link_id = ?');
        $reset->execute([(int)$_POST['reset_id']]);
        header('Location: /admin/links.php');
        exit;
    }
}
    ...
    <table>
        ...
        <?php foreach ($links as $link): ?>
            <tr>
                ...
                <td style="text-align:right; display:flex; gap:0.35rem; justify-content:flex-end; flex-wrap:wrap;">
                    <?php $shareUrl = APP_URL . '/' . urlencode($link['slug']); ?>
                    <button class="btn btn-ghost" onclick='navigator.clipboard.writeText(<?php echo json_encode($shareUrl); ?>).then(()=>alert("Đã copy"));'>Copy</button>
                    <a class="btn btn-ghost" href="/<?php echo urlencode($link['slug']); ?>" target="_blank">Xem</a>
                    <a class="btn btn-ghost" href="/admin/link_edit.php?id=<?php echo $link['id']; ?>">Sửa</a>
                    <form method="post" style="display:inline;" onsubmit="return confirm('Reset lượt click?');">
                        <input type="hidden" name="reset_id" value="<?php echo $link['id']; ?>">
                        <button class="btn btn-ghost" type="submit">Reset click</button>
                    </form>
