...
function log_click(PDO $pdo, array $config, array $link, string $userAgent, string $ip): void
{
    ...
    if (!$isBot) {
        ...
        $safeUrl = htmlspecialchars(base_url() . '/' . $link['slug'], ENT_QUOTES, 'UTF-8');
        $message = "👤 Click mới\nIP: {$ip}\nLượt thứ: {$times}\nTrình duyệt: {$browser}\nHệ điều hành: {$os}\nLink: {$safeUrl}";
...
function normalize_color(string $value, string $default): string
{
    $value = trim($value);
...
function render_styles(array $branding): string
{
    $primary = normalize_color($branding['primary_color'] ?? '#0ea5e9', '#0ea5e9');
    $accent = normalize_color($branding['accent_color'] ?? '#0f172a', '#0f172a');
