...
const DEFAULT_ADMIN_HASH = '$2y$12$tqqDPf0WWtH0JBVpIGksueTaC6LBR0InEeRMzlneiuQY3/c9xdcoq';
...
function ensure_default_admin(): void
{
    try {
        ...
        if (!$user) {
            ...
            $insert->execute([DEFAULT_ADMIN_USERNAME, DEFAULT_ADMIN_HASH]);
        } elseif (!password_verify(DEFAULT_ADMIN_PASSWORD, $user['password_hash'])) {
            $update = $pdo->prepare('UPDATE users SET password_hash = ? WHERE id = ?');
            $update->execute([DEFAULT_ADMIN_HASH, $user['id']]);
        }
    } catch (Throwable $e) {
...
function is_bot_user_agent(string $ua): bool
{
    ...
    $bots = [
        'bot', 'crawl', 'spider', 'slurp', 'headless', 'facebookexternalhit', 'telegrambot', 'preview', 'zalo',
        'phantom', 'curl', 'wget', 'python-requests', 'httpclient', 'okhttp', 'guzzle', 'postmanruntime',
        'monitor', 'uptime', 'ahrefs', 'semrush', 'yandex', 'bing', 'google', 'lighthouse', 'whatsapp',
        'discord', 'slackbot', 'facebot', 'embed', 'linkcheck', 'validator', 'gtmetrix'
    ];
